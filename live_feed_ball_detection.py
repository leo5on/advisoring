...
class circle_detection():
    ...
    def __init__(self):
        ...
        self.vid_original = cv2.VideoCapture(0)
        self.Xcenter = 0
        self.Ycenter = 0
        self.Radius = 0
    ...
    def image_treatment(self):
        ...
        cv2.namedWindow('treated_image', cv2.WINDOW_NORMAL)
        ...
        while(self.vid_original.isOpened()):
            ...
            if ret == True:
                ...
                frame_canny = cv2.Canny(frame_gray,100,300)


                frame_eq = cv2.equalizeHist(frame_canny)

                frame_blur = cv2.medianBlur(frame_eq,1)



                # parameters: 1=dp ratio, 500=min distance btw centers, param1=higher threshold, param2= accumulator threshold
                circles = cv2.HoughCircles(frame_blur,cv2.HOUGH_GRADIENT,1,700,param1=2,param2=1,minRadius=150,maxRadius=200)
                ...
                cv2.imshow('treated_image',frame_blur)
